...
// Normaliza variables para evitar notices
$buscar = $buscar ?? '';
  ...
  <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm p-6">
    ...
    <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm p-4 mb-6">
      <form class="grid grid-cols-1 md:grid-cols-4 gap-4" method="GET">
        ...
        <div>
          <label for="departamento" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Departamento</label>
          <select name="departamento" id="departamento"
            ...
            <?php foreach ($departamentos as $d): ?>
              <?php $label = is_string($d) ? $d : ((is_object($d) ? ($d->nombre ?? $d->name ?? '') : (string)$d)); ?>
              <option value="<?= $esc($label) ?>" <?= ($departamento === (string)$label) ? 'selected' : '' ?>><?= $esc($label) ?></option>
            <?php endforeach; ?>
          </select>
        </div>

        <div>
          <label for="estado" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Estado</label>
        ...
        <table class="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
          <thead class="bg-neutral-50 dark:bg-neutral-800">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">No. Empleado</th>
              <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Departamento</th>
          ...
          <tbody class="bg-white dark:bg-neutral-800 divide-y divide-neutral-200 dark:divide-neutral-700">
            <?php foreach ($teachers as $teacher): ?>
              <?php $status = (string)($teacher->status ?? ''); ?>
              <tr>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    ...
                    <div class="ml-4">
                      <div class="text-sm font-medium"><?= $esc($fullName($teacher)) ?></div>
                      <div class="text-sm text-neutral-500 dark:text-neutral-400"><?= $esc($teacher->email ?? '') ?></div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm"><?= $esc($teacher->numero_empleado ?? $teacher->num_empleado ?? '') ?></td>
                <td class="px-6 py-4 whitespace-nowrap text-sm"><?= $esc($teacher->department ?? $teacher->departamento ?? '') ?></td>
                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                  <div class="flex items-center gap-3">
                    <a href="/src/plataforma/app/admin/teachers/edit/<?= $esc($teacher->id ?? '') ?>" class="text-primary-600 hover:text-primary-900">
                      <i data-feather="edit" class="w-4 h-4"></i>
                    </a>
                    <form action="/src/plataforma/app/admin/teachers/delete/<?= $esc($teacher->id ?? '') ?>" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de que deseas eliminar este profesor?');">
            ...
            <?php if (empty($teachers)): ?>
              <tr>
                <td colspan="5" class="px-6 py-4 text-center text-neutral-500 dark:text-neutral-400">No se encontraron profesores</td>
              </tr>
            <?php endif; ?>
          </tbody>
        </table>
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between w-full">
          ...
          <div>
            <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
              <?php if ($page > 1): ?>
                <a href="?page=<?= $page - 1 ?>&q=<?= urlencode($buscar) ?>&departamento=<?= urlencode($departamento) ?>&estado=<?= urlencode($estado) ?>" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-700 text-sm font-medium text-neutral-500 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-neutral-600">
              ...
              <?php for ($i = max(1, $page - 2); $i <= min($totalPages, $page + 2); $i++): ?>
                <a href="?page=<?= $i ?>&q=<?= urlencode($buscar) ?>&departamento=<?= urlencode($departamento) ?>&estado=<?= urlencode($estado) ?>" class="relative inline-flex items-center px-4 py-2 border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-700 text-sm font-medium <?= $i === $page ? 'text-primary-600 dark:text-primary-400' : 'text-neutral-500 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-neutral-600' ?>">
                  <?= $i ?>
                </a>
              <?php endfor; ?>

              <?php if ($page < $totalPages): ?>
                <a href="?page=<?= $page + 1 ?>&q=<?= urlencode($buscar) ?>&departamento=<?= urlencode($departamento) ?>&estado=<?= urlencode($estado) ?>" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-700 text-sm font-medium text-neutral-500 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-neutral-600">
