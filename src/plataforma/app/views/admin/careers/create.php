...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($code)) {
        $stmt = $conn->prepare("SELECT COUNT(*) FROM careers WHERE code = ?");
        $stmt->execute([$code]);
    ...
    if (empty($errors)) {
        try {
            $stmt = $conn->prepare("
                INSERT INTO careers (name, code, description, duration_semesters, total_credits, modality, status)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$name, $code, $description, $duration_semesters, $total_credits, $modality, $status]);
            $success = true;
        ...
        <div class="mb-6">
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    ...
                    <p class="text-neutral-500 dark:text-neutral-400">Ingresa la información de la nueva carrera académica</p>
                </div>
            </div>
        </div>
            ...
            <form method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="block w-full rounded-md border-neutral-300 dark:border-neutral-600 dark:bg-neutral-700 focus:border-primary-500 focus:ring-primary-500"
                              placeholder="Descripción de la carrera académica"><?= htmlspecialchars($_POST['description'] ?? '') ?></textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="number" name="duration_semesters" id="duration_semesters" required
                               value="<?= htmlspecialchars($_POST['duration_semesters'] ?? '8') ?>"
                               min="1" max="12"
                               class="block w-full rounded-md border-neutral-300 dark:border-neutral-600 dark:bg-neutral-700 focus:border-primary-500 focus:ring-primary-500">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="total_credits" id="total_credits" required
                               value="<?= htmlspecialchars($_POST['total_credits'] ?? '240') ?>"
                               min="1"
                               class="block w-full rounded-md border-neutral-300 dark:border-neutral-600 dark:bg-neutral-700 focus:border-primary-500 focus:ring-primary-500">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="modality" id="modality" required
                                class="block w-full rounded-md border-neutral-300 dark:border-neutral-600 dark:bg-neutral-700 focus:border-primary-500 focus:ring-primary-500">
                            <option value="presencial" <?= ($_POST['modality'] ?? 'presencial') === 'presencial' ? 'selected' : '' ?>>Presencial</option>
                            <option value="virtual" <?= ($_POST['modality'] ?? '') === 'virtual' ? 'selected' : '' ?>>Virtual</option>
                            <option value="mixta" <?= ($_POST['modality'] ?? '') === 'mixta' ? 'selected' : '' ?>>Mixta</option>
                        </select>
                    </div>
                </div>
