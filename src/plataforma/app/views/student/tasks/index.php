...
<div class="py-8 max-w-7xl mx-auto">
    ...
    <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl p-6 text-white mb-8" data-aos="fade-up">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl font-bold mb-1">Mis Actividades</h2>
                <p class="opacity-90">Gestiona tus actividades pendientes, entregadas y vencidas</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <!-- Pendientes -->
        <div class="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-neutral-800 dark:to-neutral-800 rounded-xl shadow-lg p-6 border border-amber-200 hover:shadow-2xl hover:scale-105 transition-all duration-500 cursor-pointer group relative overflow-hidden" data-aos="fade-up" data-tooltip="Ver actividades pendientes">
        ...
        <!-- Entregadas -->
        <div class="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-neutral-800 dark:to-neutral-800 rounded-xl shadow-lg p-6 border border-green-200 hover:shadow-2xl hover:scale-105 transition-all duration-500 cursor-pointer group relative overflow-hidden" data-aos="fade-up" data-aos-delay="100" data-tooltip="Ver actividades entregadas">
        ...
        <!-- Vencidas -->
        <div class="bg-gradient-to-br from-red-50 to-pink-50 dark:from-neutral-800 dark:to-neutral-800 rounded-xl shadow-lg p-6 border border-red-200 hover:shadow-2xl hover:scale-105 transition-all duration-500 cursor-pointer group relative overflow-hidden" data-aos="fade-up" data-aos-delay="200" data-tooltip="Ver actividades vencidas">
            ...
            <div class="space-y-4">
                <?php if (!empty($pendingTasks)): ?>
                    ...
                    foreach ($pendingToShow as $task):
                        // Detectar si es examen
                        $slug = isset($task->activity_type_slug) ? (string)$task->activity_type_slug : '';
                        $isExam = ($slug === 'exam');

                        // URL según tipo
                        $taskUrl = $isExam
                            ? '/src/plataforma/app/tareas/exam/' . $task->id
                            : '/src/plataforma/app/tareas/submit/' . $task->id;
                        ...
                        $isUrgent = $daysLeft <= 2;
                    ?>
                        <a href="<?= htmlspecialchars($taskUrl) ?>" class="block">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500 dark:text-gray-300">
                                            <?= htmlspecialchars($task->course_name) ?>
                                        </p>
                                        <?php if (!empty($task->activity_type_name)): ?>
                                            <span class="inline-flex items-center mt-1 px-2 py-0.5 rounded-full text-xs font-medium
                                                <?= $isExam ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-200' : 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-200' ?>">
                                                <?= htmlspecialchars($task->activity_type_name) ?>
                                            </span>
                                        <?php endif; ?>
                                    </div>
                                </div>
                ...
                <?php else: ?>
                    <div class="text-center py-12 text-gray-500 dark:text-gray-300">
                        ...
                        <p class="text-lg">No tienes actividades pendientes</p>
