...
<head>
  ...
  <style>
    :root{
      --ut-green-900:#0c4f2e; --ut-green-800:#12663a; --ut-green-700:#177a46; --ut-green-600:#1e8c51; --ut-green-500:#28a55f;
      --ut-green-400:#4cbb7c; --ut-green-300:#7dd0a0; --ut-green-200:#b8e4c9; --ut-green-100:#e6f6ed; --ut-green-50:#f0faf4;
      --ut-gold:#d4af37;
    }
    body{ font-family:'Roboto',sans-serif; background:linear-gradient(135deg,var(--ut-green-800) 0%,var(--ut-green-700) 100%); min-height:100vh; overflow-x:hidden; }
    ...
    .shape{ position:absolute; border-radius:50%; opacity:.1; filter:blur(40px); animation:float 6s ease-in-out infinite; }
    .shape-1{ width:300px; height:300px; background:var(--ut-green-300); top:10%; right:10%; animation-delay:0s; }
    .shape-2{ width:200px; height:200px; background:var(--ut-gold); bottom:20%; left:10%; animation-delay:2s; }
    .shape-3{ width:150px; height:150px; background:var(--ut-green-100); top:50%; left:20%; animation-delay:4s; }
    @keyframes float{ 0%,100%{transform:translateY(0) rotate(0)} 50%{transform:translateY(-20px) rotate(5deg)} }
    .login-card{ backdrop-filter:blur(20px); background:rgba(255,255,255,.12); border:1px solid rgba(255,255,255,.25);
      box-shadow:0 25px 50px -12px rgba(0,0,0,.25); position:relative; z-index:2; transition:all .4s cubic-bezier(.175,.885,.32,1.275); }
    .login-card:hover{ transform:translateY(-5px); box-shadow:0 35px 60px -12px rgba(0,0,0,.35); }
    .input-field{ background:rgba(255,255,255,.15); border:1px solid rgba(255,255,255,.25); transition:all .3s ease; backdrop-filter:blur(10px); }
    .input-field:focus{ background:rgba(255,255,255,.25); border-color:rgba(255,255,255,.5); outline:none; box-shadow:0 0 0 3px rgba(40,165,95,.4); transform:translateY(-2px); }
    .input-field:hover{ transform:translateY(-1px); border-color:rgba(255,255,255,.4); }
    .btn-login{ background:linear-gradient(135deg,var(--ut-green-500) 0%,var(--ut-green-700) 100%); transition:all .3s ease; position:relative; overflow:hidden; }
    .btn-login::before{ content:''; position:absolute; top:0; left:-100%; width:100%; height:100%;
      background:linear-gradient(90deg,transparent,rgba(255,255,255,.3),transparent); transition:left .6s ease; }
    .btn-login:hover::before{ left:100%; }
    .btn-login:hover{ transform:translateY(-3px); box-shadow:0 15px 30px -10px rgba(40,165,95,.5); }
    .btn-ghost{ transition:all .3s ease; backdrop-filter:blur(10px); }
    .btn-ghost:hover{ background:rgba(255,255,255,.15); transform:translateY(-2px); box-shadow:0 10px 20px -5px rgba(255,255,255,.2); }
    .social-btn{ transition:all .3s ease; backdrop-filter:blur(10px); border:1px solid rgba(255,255,255,.2); }
    .social-btn:hover{ transform:translateY(-3px) scale(1.05); box-shadow:0 10px 20px -5px rgba(255,255,255,.3); background:rgba(255,255,255,.2); }
    .pulse-ring{ animation:pulse 2s infinite; }
    @keyframes pulse{ 0%{box-shadow:0 0 0 0 rgba(40,165,95,.7)} 70%{box-shadow:0 0 0 10px rgba(40,165,95,0)} 100%{box-shadow:0 0 0 0 rgba(40,165,95,0)} }
    .logo-container{ background:linear-gradient(135deg,rgba(255,255,255,.2),rgba(255,255,255,.1)); border:2px solid rgba(255,255,255,.3); backdrop-filter:blur(10px); }
    ...
    .gradient-text{ background:linear-gradient(135deg,var(--ut-green-300),var(--ut-gold),var(--ut-green-100)); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
  </style>
</head>
<body>
    ...
    <div class="login-card rounded-3xl overflow-hidden w-full max-w-md">
      <div class="bg-white/10 p-8 relative overflow-hidden">
        <div class="text-center mb-8 relative z-10">
          ...
          <p class="text-white/90 mt-3 text-lg">Sistema de Acceso</p>
        </div>
        ...
        <!-- AHORA se usa identificador (matrícula o número de empleado) -->
        <form class="space-y-6" method="post" action="/src/plataforma/login" autocomplete="on">
          <div class="space-y-4">
            <div>
              <label for="identificador" class="block text-sm font-medium text-white/90 mb-2">
                Matrícula o No. de empleado
              </label>
              <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <i data-feather="user" class="text-white/70 floating-icon"></i>
                </div>
                <input
                  type="text"
                  id="identificador"
                  ...
                  placeholder="Ej.: A23-00123 ó EMP-0456">
              </div>
              <p class="mt-2 text-xs text-white/70">
                Alumnos: usa tu <strong>matrícula</strong>. &nbsp;Personal: tu <strong>número de empleado</strong>.
              </p>
            </div>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      inputs.forEach((input, index) => {
        setTimeout(() => { input.style.transform='translateY(0)'; input.style.opacity='1'; }, index*100);
      });
    });
  </script>
</body>
