...
<div class="container px-6 py-8">
    <div class="max-w-3xl mx-auto">
        <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm p-6">
            ...
            <form action="/src/plataforma/app/teacher/grades/update/<?= $grade['id'] ?>" method="POST" class="space-y-6">
                            ...
                            <select name="student_id" required class="w-full px-4 py-2 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800">
                                ...
                                <?php if (!empty($students)): ?>
                                    <?php foreach ($students as $student): ?>
                                        <option value="<?= $student['id'] ?>" <?= $student['id'] == $grade['student_id'] ? 'selected' : '' ?>><?= htmlspecialchars($student['name']) ?> - <?= htmlspecialchars($student['matricula'] ?? $student['id']) ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
                            ...
                            <select name="subject_id" required class="w-full px-4 py-2 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800">
                                ...
                                <?php if (!empty($subjects)): ?>
                                    <?php foreach ($subjects as $subject): ?>
                                        <option value="<?= $subject['materia_id'] ?>" <?= $subject['materia_id'] == $grade['assignment_id'] ? 'selected' : '' ?>><?= htmlspecialchars($subject['name']) ?> (<?= htmlspecialchars($subject['code']) ?>)</option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
